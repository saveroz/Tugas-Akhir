...
#include <WiFi101.h>
#endif
char ssid[] = "Rukmi";    //  your network SSID (name)
char pass[] = "Idwan1962";   // your network password
...
void loop() {
  ...
  float gelombang_tsunami = ThingSpeak.readFloatField(myChannelNumber, 1, myReadAPIKey);
  int mode_system = ThingSpeak.readIntField(myChannelNumber, 6, myReadAPIKey);
  if (mode_system == 1) {
    if (gelombang_tsunami != 0) {
