...
#include <WiFi101.h>
#endif
char ssid[] = "Orevas";    //  your network SSID (name)
char pass[] = "0721253062";   // your network password
...
*/
#define alarm D5
unsigned long myChannelNumber = 422551;
unsigned long myChannelNumber2 = 538429;

const char * myReadAPIKey = "S259TULEZEFWDQYM";
const char * myReadAPIKey2 = "TG1XSZQLF22SZ3SK";
 // pin alarm
void setup() {
...
void loop() {
  ...
  float gelombang_tsunami = ThingSpeak.readFloatField(myChannelNumber, 6, myReadAPIKey);
  int mode_system = ThingSpeak.readIntField(myChannelNumber2, 1, myReadAPIKey2);
  float skalaMMI=ThingSpeak.readIntField(myChannelNumber, 2, myReadAPIKey);
  if (mode_system == 1 && (skalaMMI > 3 || gelombang_tsunami > 3))
  { 
  //   digitalWrite(alarm, HIGH);
//  delay(200);
      digitalWrite(alarm, LOW);
      Serial.println(gelombang_tsunami);
      Serial.println(mode_system);
      Serial.println(skalaMMI);
      Serial.println("Tsunami has been detected");
      delay(100);
     // digitalWrite(alarm, HIGH);
